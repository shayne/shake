...
char *makescriptname2(char *path) {
    ...
    unsigned int prefpos = (unsigned int)config.cmd_prefix_len;
...
ssize_t loadCommands(char ***cmds_out, char ***descs_out) {
    ...
    size = fncount + (ssize_t)globbuf.gl_pathc;
    ...
    for (i = 0; i < fncount; i++) {
        int off = i + (int)globbuf.gl_pathc;
        check(fns[i], "null fn");
        char *cmd = strdup(fns[i]);
        check_mem(cmd);
        cmds[off] = cmd;
        if (fndescs[i]) descs[off] = strdup(fndescs[i]);
    }
